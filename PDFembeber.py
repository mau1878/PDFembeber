...
def main():
    ...
    with st.form(key=f"pdf_form_{st.session_state.form_key}"):
        ...
        if submit:
            ...
            elif operation == "Merge PDFs" and not additional_files:
                st.error("Please upload at least one additional PDF for merging.")
                debug_logs.append(f"[{time.strftime('%H:%M:%S')}] Error: No additional PDFs uploaded for merging")
            else:
                try:
                    main_pdf_data = io.BytesIO(main_pdf.read())
                    additional_files_data = [(io.BytesIO(file.read()), file.name) for file in additional_files] if additional_files else []
